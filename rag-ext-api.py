...
class MistralClient:
    ...
    def answer_with_tools(self, question:str, ) -> str:
        ...
        tools = [
            {
                ...
                "function": {
                    "name" : "search",
                    "description" : "search duck duck go api for factual information",
                    "parameters" : {
                        "type": "object",
                        "properties": {
                            "query": {"type": "string"},
        ...
        for tc in tool_calls:
            ...
            fn_agrs = json.loads(tc.function.arguments or "{}")

            tool_fn = TOOL_REGISTRY.get(fn_name, None)
            if not tool_fn:
                raise RuntimeError(f"Unsupported tool requested: {fn_name} - {tool_fn}")

            tool_output = tool_fn(**fn_agrs)
            print("==============================================")
            print(tool_output)
            messages.append(
                {
        ...
        for m in messages:
            if m.get("role") == "tool":
                tool_blocks.append(f"Tool result:\n{build_context_from_tool_json(m['content'])}")
